...
CREATE TABLE IF NOT EXISTS users (
    ...
    locked_until DATETIME DEFAULT NULL COMMENT 'Account lock expiry (NULL = not locked)'
);
...
('admin', '$2b$12$EBp8whTw2v0WezYX8XazqO9oN9cVESRoCnWCjscgvsUcmGTLtUJJe');

-- Insert sample data (for testing)
INSERT IGNORE INTO sources (name, type, secret_key, created_by_user_id) VALUES
('Synology NAS', 'synology', 'h7k9m2x_secret_key_12345', 1),
('Proxmox Server', 'proxmox', 'n4p8w6z_secret_key_67890', 1);

INSERT IGNORE INTO targets (name, type, webhook_url, created_by_user_id) VALUES
('Test Mattermost', 'mattermost', 'https://mattermost.example.com/hooks/test123', 1);

INSERT IGNORE INTO routes (source_id, target_id, message_template, created_by_user_id) VALUES
(1, 1, 'Alert from {{source}}: {{message}}', 1);

-- Insert default settings
INSERT IGNORE INTO system_settings (setting_key, setting_value, setting_type, description, is_sensitive) VALUES
('app_name', 'Webhook Server', 'string', 'Application display name', FALSE),
('jwt_expiry', '24h', 'string', 'JWT token lifetime', FALSE),
('session_timeout', '3600', 'number', 'Session timeout in seconds', FALSE),
('webhook_retry_attempts', '3', 'number', 'Number of webhook delivery attempts', FALSE),
('webhook_signature_validation', 'true', 'boolean', 'Enable webhook signature validation', FALSE),
('sso_enabled', 'false', 'boolean', 'Enable SSO login', FALSE),
('sso_only', 'false', 'boolean', 'Only SSO login allowed (local login disabled)', FALSE),
('sso_provider', 'authentik', 'string', 'SSO provider type (only Authentik supported)', FALSE),
...
('sso_scopes', 'openid profile email', 'string', 'SSO requested scopes', FALSE),
('timezone', 'Europe/Budapest', 'string', 'Default timezone', FALSE),
('rbac_enabled', 'true', 'boolean', 'Enable role-based access control', FALSE),
('min_admin_count', '1', 'number', 'Minimum number of admin users', FALSE),
('user_registration_enabled', 'false', 'boolean', 'Enable new user registration', FALSE),
('default_user_role', 'user', 'string', 'Default role for new users', FALSE);
